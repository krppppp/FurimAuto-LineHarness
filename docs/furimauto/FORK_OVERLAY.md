...
- apps/worker/src/services/ad-conversion.ts — `sendAdConversions` の先頭で、検証用アカウント（furim/segments.ts の `TEST_LINE_IDS`）なら送信しない（import 1 行＋数行・2026-09-17 #301。あじゃぱーの実機テストが Google 広告の成果として送られたため） | 数行を再注入
