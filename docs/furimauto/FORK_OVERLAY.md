...
- apps/worker/src/routes/webhook.ts — 【プラン変更】の dispatch は `runHandlerSafely` で包む（2026-09-13 #240）。follow/unfollow で `fireEvent`。メッセージ振り分け（【ボタン】【キーワード】・リッチメニュー切替・AIチャット・Furimanクーポン・解説見た）。handler は全て furim/ 側。WebhookEnv 型 | upstream の follow/message ハンドラに furim handler 呼び出しを再注入
