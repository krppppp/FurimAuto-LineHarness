...
--
-- ■ 実行済みの節（権限管理課）※ 流し直さないこと。ops_log の id が主キーで衝突する
--   - 2026-09-17 16:1x 頃: 89f0da0 のデプロイ（Worker 978f6aa6）のあと、1-a・1-b（ops_log …0001）・2（…0002）・3（…0003）・4（…0004）
--   - 2026-09-17 16:37〜16:39: 97ad553 のデプロイ（Worker efb7c502）のあと、1-c（ops_log …0005）・6（…0006）
--   - 5 は SQL なし
--   - 2026-09-17 の初版では、1-a・1-b の締めの記録（…0001）が 1-c の節の末尾に入っていて、節の単位で流すと主キーが衝突した。
--     権限管理課はその 1 文を外して流した。いまの版では 1-b の直後に移してある（本番で流し直す必要は無い）
--
-- ■ 実行の順番（重要）
...
-- 1-c の実行後の期待値:
--   SELECT COUNT(*) FROM chats WHERE status = 'resolved' AND friend_id IN ('c28683b3-331f-46a7-9487-94d846f7498b','5255275b-6011-4084-b5e7-147f0d206318','66f2aa77-959c-43e4-8544-02a103e7ea67','a9b8e5f7-0666-49e4-8a2e-c3af53ac69de','b2ab1365-4e17-41e6-9c77-bd092b0e81ed','e6e88056-65b1-42b1-acb6-181aff7e2b3f','f33b27aa-2877-4cd3-9968-a191b76c6ddf');
--   → 7（一覧作成後に新着があって飛ばした友だちがいれば、その数だけ少ない）
...
-- ════════════════════════════════════════════════════════════════════
-- 6. 追加（2026-09-17 16:25 頃・97ad553）: あじゃぱーの差分 4 件（16:15:52 の突き合わせで新しく出たもの）→ D1 が正しい
-- ════════════════════════════════════════════════════════════════════
--
-- ■ 判断（統括）: 16:07 の実機テストで友だちを作り直した D1 の値が正しく、凍結中のシートの古い行との食い違い。
-- ■ 再発防止: このファイルと同じ依頼で出すコミットで、検証用アカウント（segments.ts の TEST_LINE_IDS・あじゃぱー）を
--   シートと D1 の突き合わせから外した。デプロイ後の最初の突き合わせ（JST :15 / :45）で、下の 4 件は
--   「観測されなくなった差分」として自動で resolved になる。その後に流すと下の UPDATE は 0 行で終わる（正常）。
--   デプロイ前に流した場合は、accepted: の受け入れ（90308e2）によって作り直されない。どちらの順でも結果は同じ。
--
-- ── 実行前の確認 ────────────────────────────────────────────────
--   SELECT id, line_user_id, field, d1_value, sheet_value, resolved_at FROM furim_sync_diffs WHERE resolved_at IS NULL;
--   → 4 行（下の id）、line_user_id はすべて Ue4941a030cb2ec8758095fb0fffff344。ほかの行があれば止めて統括に連絡
--     （デプロイ後に突き合わせが走っていれば 0 行。そのときは下を流さなくてよい）
UPDATE furim_sync_diffs
SET resolved_at = strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours') || '+09:00',
    notified_at = 'accepted:sheet_side:検証用アカウント（あじゃぱー）の実機テストで D1 を作り直した。シートは凍結の古い行'
WHERE resolved_at IS NULL AND line_user_id = 'Ue4941a030cb2ec8758095fb0fffff344'
  AND id IN ('e29b6aef-6d75-41ce-8997-fe2242ee3e30', 'eddc9896-67d5-4662-b1a9-23696cef0d9a', '3fc1a702-eee9-48b1-988e-c2d80a43b891', 'a713fad2-60db-433c-b55e-f3a0e3036976');

INSERT INTO furim_ops_log (id, acted_at, actor, action, target, target_count, goal, note)
VALUES ('0917a001-0000-4000-8000-000000000006', strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours') || '+09:00', '権限管理課', 'accept_sheet_side', 'furim_sync_diffs', 4, 'Capsec #301',
        'あじゃぱー（検証用）の 16:07 実機テストで D1 を作り直したことによる差分 4 件。D1 が正しい。同じ依頼のコミットで検証用アカウントを突き合わせから外した');

-- ── 実行後の期待値 ──────────────────────────────────────────────
--   SELECT COUNT(*) FROM furim_sync_diffs WHERE resolved_at IS NULL;   -- 期待値 0
--   トップの異常区画から「シートと D1 の差分」が消え、次のテストで友だちを作り直しても出ない
