...
## シナリオ体系（2026-04-03 8段階対応）

**7シナリオ構成（通常/紹介を統合）**

通常・紹介ともに同じシナリオを使用。1日=1ステップ、毎朝9時配信。
試用期間の差（7日/14日）はGAS側のセグメント切り替えタイミングで吸収する。
isReferral による区別なし（`SCENARIO_NAME_MAP` はセグメント番号のみをキーとする）。
...
| FurimAuto セグメント5: Free30未取得 | `manual` | 5（Day0〜4） |
| FurimAuto セグメント6: Free30取得済み | `manual` | 7（Day0〜6） |
| FurimAuto セグメント7: Youtubeクーポン取得済み | `manual` | 4（Day0〜3） |
| seg8 | — | シナリオなし（kaisetsu_daily オートメーションで管理） |
