...
// シナリオ名マップ: `${segment}_${isReferral}` → DB上のシナリオ名
const SCENARIO_NAME_MAP: Record<string, string> = {
  '1_false': 'FurimAuto 通常 ステップ配信（セグメント1: アンケート未回答）',
  '2_false': 'FurimAuto 通常 ステップ配信（セグメント2: アンケート回答済み）',
  '3_false': 'FurimAuto 通常 ステップ配信（セグメント3: キーコード発行済み）',
  '4_false': 'FurimAuto 通常 ステップ配信（セグメント4: 拡張インストール済み）',
  '5_false': 'FurimAuto 通常 ステップ配信（セグメント5: メルカリURL登録済み）',
  '6_false': 'FurimAuto 通常 ステップ配信（セグメント6: FREEコピー出品チケット取得）',
  '7_false': 'FurimAuto 通常 ステップ配信（セグメント7: Youtubeクーポン取得）',
  '1_true':  'FurimAuto 紹介 ステップ配信（セグメント1: アンケート未回答）',
  '2_true':  'FurimAuto 紹介 ステップ配信（セグメント2: アンケート回答済み）',
  '3_true':  'FurimAuto 紹介 ステップ配信（セグメント3: キーコード発行済み）',
  '4_true':  'FurimAuto 紹介 ステップ配信（セグメント4: 拡張インストール済み）',
  '5_true':  'FurimAuto 紹介 ステップ配信（セグメント5: メルカリURL登録済み）',
  '6_true':  'FurimAuto 紹介 ステップ配信（セグメント6: FREEコピー出品チケット取得）',
  '7_true':  'FurimAuto 紹介 ステップ配信（セグメント7: Youtubeクーポン取得）',
};
...
furim.post('/api/furim/scenario-switch', async (c) => {
  try {
    ...
    const key = `${body.segment}_${!!body.isReferral}`;
    const scenarioName = SCENARIO_NAME_MAP[key];
    if (!scenarioName) {
      return c.json({ success: false, error: `Unknown segment/isReferral: ${key}` }, 400);
    }
