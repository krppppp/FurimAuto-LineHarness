...
describe('restriction-event（利用制限の検知・Capsec #293）', () => {
  it('許可した機能名だけを残し、それ以外の語や本文は保存しない', async () => {
    const { db, statements } = makeDb((sql) =>
      /SELECT line_user_id FROM furim_customers WHERE key_code/.test(sql) ? { first: { line_user_id: 'U1' } } : undefined,
    );
    const res = await call(envWith(db), 'restriction-event', {
      keyCode: 'pb_x',
      service: 'メルカリ',
      restrictedFeatures: ['出品', 'コメント', '山田太郎', '東京都港区1-2-3', '090-0000-0000', '出品'],
      message: '【制限されている機能】出品・コメント 山田太郎様 東京都港区1-2-3 090-0000-0000',
      accountUrl: 'https://jp.mercari.com/user/profile/1',
    });
    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({ success: true, recorded: true });
    const ins = statements.find((x) => /INSERT INTO furim_ext_errors/.test(x.sql))!;
    const saved = ins.args.map((a) => String(a)).join(' | ');
    expect(saved).toContain('aclException / メルカリ / 出品・コメント');
    // 個人情報になりうるものは、どの列にも入らない
    expect(saved).not.toContain('山田太郎');
    expect(saved).not.toContain('港区');
    expect(saved).not.toContain('090-0000-0000');
    expect(saved).not.toContain('【制限されている機能】');
    expect(saved).not.toContain('profile/1');
  });

  it('同じ日の同じ制限は 2 行目を入れない', async () => {
    const { db, statements } = makeDb((sql) =>
      /FROM furim_ext_errors\s+WHERE method = 'aclException'/.test(sql) ? { first: { 1: 1 } } : undefined,
    );
    const res = await call(envWith(db), 'restriction-event', { keyCode: 'pb_x', service: 'メルカリ', restrictedFeatures: ['出品'] });
    expect(await res.json()).toMatchObject({ success: true, message: '同じ日の同じ制限は記録済み' });
    expect(statements.some((x) => /INSERT INTO furim_ext_errors/.test(x.sql))).toBe(false);
  });

  it('許可した語が 1 つも無ければ「機能不明」として残す（本文は入れない）', async () => {
    const { db, statements } = makeDb(() => undefined);
    await call(envWith(db), 'restriction-event', { keyCode: 'pb_x', service: 'メルカリ', restrictedFeatures: ['よく分からない文'] });
    const ins = statements.find((x) => /INSERT INTO furim_ext_errors/.test(x.sql))!;
    expect(ins.args.map(String).join(' | ')).toContain('(機能不明)');
    expect(ins.args.map(String).join(' | ')).not.toContain('よく分からない文');
  });

  it('keyCode が無ければ 400', async () => {
    const { db } = makeDb(() => undefined);
    const res = await call(envWith(db), 'restriction-event', { service: 'メルカリ' });
    expect(res.status).toBe(400);
  });
});
