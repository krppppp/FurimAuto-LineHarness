...
import { furimCronPrelude, furimCronJobs, furimCronSixHourly } from './furim/cron.js';
...
export async function scheduled(
  ...
  // FurimAuto: 独自の定期ジョブ (furim/cron.ts に集約)
  furimCronPrelude(env, ctx);
  ...
  jobs.push(...furimCronJobs(env, defaultLineClient)); // FurimAuto
  ...
  if (event.cron === '0 */6 * * *') {
    await furimCronSixHourly(env); // FurimAuto
    try {
