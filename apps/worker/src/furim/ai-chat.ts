...
async function generateAIResponse(queryText: string, env: AIChatEnv): Promise<{ text: string; additionalMessages: unknown[] }> {
  const contextText = await fetchSpecFiles(env.GITHUB_PAT);
  if (!contextText) {
...
export async function handleAIChat(
  ...
  const { text: aiReply, additionalMessages } = await generateAIResponse(text, env);
