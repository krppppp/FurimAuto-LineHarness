 ...
 *
 * ⚠️ 注意（2026-08-27の実事故）: 対象が有効なStripeサブスクを持っていると、顧客削除に伴う
 * subscription.deleted Webhookで「月額解約フロー」(12アクション)が発火し、解約系メッセージが
 * 本人のLINEに一斉送信される。さらにWebhook処理がリセットと競合すると友だち行が残って
 * リフォロー扱いになる。有料サブスク持ちを検証に使う場合は:
 *   リセット実行 → 1分待つ → もう一度リセット（Webhook残骸の掃除） → ブロック→解除
 *
 * 安全装置:
