...
// 箇条書き1行（アイコン列＋テキスト列の2カラム。折返してもテキストの頭が揃う・2026-08-27くろさんFB）
function listRow(icon, text, { size = 'sm', color = '#555555', weight } = {}) {
  return {
    type: 'box', layout: 'horizontal', spacing: 'sm', margin: 'xs',
    contents: [
      t(icon, { flex: 0, size, color, ...(weight ? { weight } : {}) }),
      t(text, { flex: 1, size, color, ...(weight ? { weight } : {}) }),
    ],
  };
}
// 色付きボックス内の箇条書きブロック
function hlRows(rows, bg = '#FFF1EC', { header } = {}) {
  return {
    type: 'box', layout: 'vertical', backgroundColor: bg, cornerRadius: '8px', paddingAll: '12px', spacing: 'xs',
    contents: [...(header ? [header] : []), ...rows],
  };
}
...
const set2b = flexMsg('🎁 無料期間中に15大特典をGETしよう！', bubble({
  ...
  body: [
    ...
    { type: 'separator', margin: 'md' },
    t('🔓 使うほどもらえる特典', { weight: 'bold', color: '#333333', margin: 'md' }),
    t('③④ ロードマップ❸❹\n⑤⑥ 撮影方法マニュアル 前編・後編\n⑦⑧ 外注化マニュアル 前編・後編\n⑨⑩ 外注募集テンプレ・業務委託契約書', { size: 'xs', color: '#444444' }),
    t('🎬 動画視聴＋キーワード入力でもらえる特典', { weight: 'bold', color: '#333333', margin: 'md' }),
...
const d1n = flexMsg('【15大特典への道 1/6】今日のミッション: 1問アンケートに回答（30秒）', bubble({
  ...
  label: '15大特典への道 1/6',
  body: [
    t('FurimAutoです🎁\n\n無料期間中に集められる【15大特典】、ご存知ですか？\n①②の物販ロードマップは、リッチメニューの「限定特典GET」からいますぐ受け取れます。'),
    hlBox('今日のミッション🎯\n▶ 1問アンケートに回答（30秒）', '#FFF6E5', '#E8730C', 'sm'),
    t('クリアでもらえる特典', { margin: 'md' }),
    listRow('📘', '③ ロードマップ❸'),
    listRow('📘', '④ ロードマップ❹'),
...
const d2n = flexMsg('【15大特典への道 2/6】今日のミッション: キーコードを発行する（無料・30秒）', bubble({
  ...
  body: [
    ...
    t('クリアでもらえる特典', { margin: 'md' }),
    listRow('📕', '⑤ 撮影方法マニュアル 前編'),
    listRow('📕', '⑥ 撮影方法マニュアル 後編'),
    t('"売れる写真"の撮り方を体系化した教材です。\n出品の質が、今日から変わります。\n\n発行は下のボタンから。特典はリッチメニューの「限定特典GET」から受け取れます👇', { margin: 'md' }),
  ],
...
const d4n1 = flexMsg('【15大特典への道 4/6】今日のミッション: 無料チケット30枚を受け取ってコピー出品を試す', bubble({
  ...
  label: '15大特典への道 4/6',
  body: [
    t('昨日予告した【全自動コピー出品】、その無料チケット30枚を今日配ります🎫'),
    hlBox('今日のミッション🎯\n▶ 無料チケット30枚を受け取って\n　自動コピー出品を試す', '#FFF6E5', '#E8730C', 'sm'),
    t('クリアでもらえる特典', { margin: 'md' }),
    listRow('📄', '⑨ 外注募集テンプレート'),
    listRow('📄', '⑩ 外注先業務委託契約書テンプレ'),
    t('昨日のマニュアルを"今日から実行"できる実物のテンプレートです。\n\nチケットは下のカードから、特典はリッチメニューの「限定特典GET」から👇', { margin: 'md' }),
  ],
}));
...
const d5m = flexMsg('【1つのサイトで回ったら、次のステージへ】販路拡大はコピー出品で', bubble({
  ...
  body: [
    ...
    listRow('・', '手動のワンクリックコピーは、無料で使えます'),
    listRow('・', '全自動の連続出品は、1品あたり10〜15円のチケット制（Day4で配った無料30枚も使えます）'),
...
const d5n = flexMsg('【15大特典への道 5/6】今日のミッション: YouTube動画講座を見てキーワードを送る', bubble({
  ...
  body: [
    ...
    t('クリアでもらえる特典', { margin: 'md' }),
    listRow('📙', '⑪ コメントセールの手法と効果の解説'),
    t('商品にいいねした人へ"追いセールコメント"を打つ手法を深掘りした資料です。値引き幅の考え方まで分かります。\n\n動画講座は下のボタンからどうぞ👇', { margin: 'md' }),
    bimg(GCS_IMG + 'youtube_coupon.png', '16:9', 'md'),
    hlRows([
            listRow('・', '友達登録から1週間以内 → 初月半額クーポン', { color: '#D94A3D', weight: 'bold' }),
            listRow('・', 'それ以外 → 初月20%OFFクーポン', { color: '#D94A3D', weight: 'bold' }),
            t('をそれぞれプレゼントいたします！（初回決済に適用）', { weight: 'bold', size: 'sm', color: '#D94A3D', margin: 'sm' }),
          ], '#FFF1EC', { header: t('【お得に使えるクーポンをGET!!】\n動画内のキーワードをLINEに送っていただいた方には、', { weight: 'bold', size: 'sm', color: '#D94A3D' }) }),
  ],
  footer: [ytBtn('初月半額クーポンを獲得！')],
}));
...
const d6m = flexMsg('【最後の壁は、在庫管理】自動併売在庫管理で売り違いは構造的に起きません', bubble({
  ...
  body: [
    ...
    t('売り違いの恐怖から解放されて、あなたがやるのは仕入れて、出品して、発送するだけ。\n\nリサーチ → 出品 → 自動化 → 販路拡大 → 在庫管理。\n\nこの2週間でご案内した階段をのぼり切ると、"片手間でも、ちゃんと伸ばせる物販"の形が完成します。', { margin: 'md' }),
    hlBox('その形、このまま続けませんか？\n無料期間の終盤に、継続をご希望の方へのご案内をお送りします😊', '#FFF6E5', '#333333', 'sm'),
  ],
  footer: [ytBtn('在庫管理の始め方を見る')],
}));

// Day6 13:00
const d6n = flexMsg('【15大特典への道 6/6】特典への道、最終日。いちばん大きい特典です', bubble({
  color: COLORS.tokuten,
  label: '15大特典への道 6/6',
  body: [
    t('特典への道、最終日です。いちばん大きい特典をどうぞ。'),
    hlBox('今日のミッション🎯\n▶ 完全解説動画を見て、動画内で案内される\n　キーワードをこのLINEに送る', '#FFF6E5', '#E8730C', 'sm'),
    t('クリアでもらえる特典', { margin: 'md' }),
    listRow('📒', '⑫ 売れるブランドリスト'),
    listRow('📒', '⑬ 売れるアカウント説明&プロフィール解説'),
    ...
    messages: [
      closingMsg('【現在地の確認です】無料期間は残り5日になりました', bubble({
        ...
        body: [
          ...
          hlRows([
            listRow('✅', 'リサーチで売れるものが分かる（無料）', { color: '#333333', weight: 'bold' }),
            listRow('✅', '出品まわりの補助機能（無料）', { color: '#333333', weight: 'bold' }),
            listRow('⬜', '値下げ・再出品などの自動化（無料期間中に体験できます）', { color: '#333333', weight: 'bold' }),
            listRow('⬜', 'コピー出品での販路拡大', { color: '#333333', weight: 'bold' }),
            listRow('⬜', '併売の在庫管理', { color: '#333333', weight: 'bold' }),
          ], '#FFF6E5'),
    ...
    messages: [
      closingMsg('【登録は、思っているより簡単です】最短3分で自動化がそのまま続きます', bubble({
        ...
        body: [
          ...
          hlRows([
            listRow('①', 'プラン診断で自分に合うプランを確認（1分）', { color: '#333333', weight: 'bold' }),
            listRow('②', 'そのまま画面から申し込み', { color: '#333333', weight: 'bold' }),
            listRow('③', 'クレジットカード決済で完了', { color: '#333333', weight: 'bold' }),
          ], '#FFF6E5'),
