#!/usr/bin/env node
/**
 * 【古い台本・実行しないこと】無料試用が 7 日だった時代のもの。今の本番は 2 週間（14 日）で、
 * 配信の設計も統合版（specs/unified-step-scenario.md）に変わっている。実行すると古い設計の
 * シナリオ・テンプレートを作ってしまう。本番の正は scripts/furimauto-unified-flex.mjs。
 * 記録として残しているだけで、直して使う対象ではない（Capsec #311・2026-09-18 統括判断）。
 * 中の「7日間」「1週間」もそのまま残す（数字だけ直すと使える台本に見えてしまうため）。
 */
/**
 * FurimAuto ステップ配信シナリオ登録スクリプト
 * friend_add トリガー / セグメント1（アンケート未回答 7日間）
 *
 * 使い方:
 *   WORKER_URL=https://line-harness.furimuato.workers.dev API_KEY=xxx node scripts/seed-furimauto-scenario.mjs
 */
...
const STEPS = [
  ...
  },
  {
    ...
    delayMinutes: 0,
    messageType: 'text',
    messageContent: `【無料お試し期間スタート！特典①②は受け取り済みです🎁】
...
async function main() {
  ...
  const scenario = await post('/api/scenarios', {
    name: 'FurimAuto 友達追加 ステップ配信（セグメント1: アンケート未回答）',
    triggerType: 'friend_add',
    isActive: false, // 確認後に有効化
  });
  ...
  console.log('※ isActive=false で作成済み。動作確認後に管理UIまたはAPIで有効化してください。');
  console.log(`  curl -X PUT ${BASE_URL}/api/scenarios/${scenario.id} -H 'X-API-Key: ...' -d '{"isActive":true}'`);
}
